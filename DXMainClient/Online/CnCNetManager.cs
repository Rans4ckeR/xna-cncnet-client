...
public class CnCNetManager : IConnectionManager
{
    ...
    public void AddChannel(Channel channel)
    {
        if (FindChannel(channel.ChannelName) != null)
            throw new ArgumentException("The channel already exists!".L10N("UI:Main:ChannelExist"), nameof(channel));
    ...
    {
        connection.QueueMessage(QueuedMessageType.SYSTEMMESSAGE, 10, "PART " + channel.ChannelName);
        ...
        if (message.Contains("ACTION"))
        {
            ...
            for (int i = 0; i < gameCollection.GameList.Count; i++)
            {
                ...
                message = message.Replace(
                    "new " + gameCollection.GetGameIdentifierFromIndex(i) + " game",
        ...
        else
        {
            ...
            if (message.Contains(Convert.ToString((char)03)))
            {
                ...
                else
                {
                    ...
                    foreColor = colorIndex < ircChatColors.Length && colorIndex > -1 ? ircChatColors[colorIndex].XnaColor : cDefaultChatColor;
                }
            }
            else
            {
    ...
    {
        CnCNetPrivateMessageEventArgs e = new(sender, message);
    ...
    private void DoUserJoinedChannel(string channelName, string host, string userName, string userAddress)
    {
        ...
        ChannelUser channelUser = new(ircUser)
        {
            IsAdmin = isAdmin
        };
    ...
    private void DoUserListReceived(string channelName, string[] userList)
    {
        ...
        List<ChannelUser> channelUserList = new();
    ...
    private void DoWhoReplyReceived(string ident, string hostName, string userName, string extraInfo)
    {
        ...
        IRCUser user = UserList.Find(u => u.Name == userName);
        if (user != null)
        {
