...
public class FileHashCalculator
{
    ...
    public void CalculateHashes(List<GameMode> gameModes)
    {
        ...
        {
            fh.INIHashes = FileHashCalculator.AddToStringIfFileExists(fh.INIHashes, filePath);
        ...
        foreach (string path in iniPaths)
        {
            if (!string.IsNullOrEmpty(path) && Directory.Exists(path))
            {
                List<string> files = Directory.GetFiles(path, "*", SearchOption.AllDirectories).
                    Select(s => s.Replace(ProgramConstants.GamePath, string.Empty).Replace("\\", "/")).ToList();
    ...
    private static string AddToStringIfFileExists(string str, string path)
    {
        if (File.Exists(path))
    ...
    {
        IniFile config = new(ProgramConstants.GetBaseResourcePath() + CONFIGNAME);
