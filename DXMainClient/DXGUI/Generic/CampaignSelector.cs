...
#if NETFRAMEWORK
            using (spawnStreamWriter)
#else
            await using (spawnStreamWriter.ConfigureAwait(true))
#endif
            {
                await spawnStreamWriter.WriteLineAsync("; Generated by DTA Client").ConfigureAwait(true);
                await spawnStreamWriter.WriteLineAsync("[Settings]").ConfigureAwait(true);
                if (copyMapsToSpawnmapINI)
                    await spawnStreamWriter.WriteLineAsync($"Scenario={ProgramConstants.SPAWNMAP_INI}").ConfigureAwait(true);
                else
                    await spawnStreamWriter.WriteLineAsync("Scenario=" + mission.Scenario).ConfigureAwait(true);
                ...
                await spawnStreamWriter.WriteLineAsync("CampaignID=" + mission.Index).ConfigureAwait(true);
                await spawnStreamWriter.WriteLineAsync("GameSpeed=" + UserINISettings.Instance.GameSpeed).ConfigureAwait(true);
#if YR || ARES
                await spawnStreamWriter.WriteLineAsync("Ra2Mode=" + !mission.RequiredAddon).ConfigureAwait(true);
#else
                ...
                await spawnStreamWriter.WriteLineAsync("BuildOffAlly=" + mission.BuildOffAlly).ConfigureAwait(true);
