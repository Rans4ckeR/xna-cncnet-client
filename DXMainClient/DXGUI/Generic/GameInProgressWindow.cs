...
public class GameInProgressWindow : XNAPanel
{
    ...
    public override void Initialize()
    {
        ...
        XNAWindow window = new(WindowManager)
        {
            Name = "GameInProgressWindow",
            BackgroundTexture = AssetLoader.LoadTexture("gameinprogresswindowbg.png"),
            ClientRectangle = new Rectangle(0, 0, 200, 100)
        };

        XNALabel explanation = new(WindowManager)
        {
            Text = "A game is in progress.".L10N("UI:Main:GameInProgress")
        };
    ...
    {
#if ARES
        debugSnapshotDirectories = GameInProgressWindow.GetAllDebugSnapshotDirectories();
#else
        try
        {
    ...
    private void HandleGameProcessExited()
    {
        ...
        snapshotDirectory ??= ProgramConstants.GamePath + "debug/snapshot-" +
        ...
        if (GameInProgressWindow.CopySyncErrorLogs(snapshotDirectory, null) || snapshotCreated)
        {
            ...
            GameInProgressWindow.CopyErrorLog(snapshotDirectory, "syringe.log", null);
        }
#else
