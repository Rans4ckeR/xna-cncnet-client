...
{
    public TeamStartMappingsPanel(WindowManager windowManager)
        : base(windowManager)
    {
        DrawBorders = false;
    }
    ...
    {
        List<TeamStartMappingPanel> teamStartMappingPanels = GetTeamStartMappingPanels();
        for (int i = 0; i < teamStartMappingPanels.Count; i++)
        {
            ...
            TeamStartMapping teamStartMapping = teamStartMappings[i];
