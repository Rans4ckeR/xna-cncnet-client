...
public class GameInformationPanel : XNAPanel
{
    ...
    public override void Initialize()
    {
        ...
        {
            FontIndex = 1,
            Text = "GAME INFORMATION".L10N("UI:Main:GameInfo")
        };

        lblGameMode = new XNALabel(WindowManager)
        {
            ClientRectangle = new Rectangle(6, 30, 0, 0)
        };

        lblMap = new XNALabel(WindowManager)
        {
            ClientRectangle = new Rectangle(6, 54, 0, 0)
        };

        lblGameVersion = new XNALabel(WindowManager)
        {
            ClientRectangle = new Rectangle(6, 78, 0, 0)
        };

        lblHost = new XNALabel(WindowManager)
        {
            ClientRectangle = new Rectangle(6, 102, 0, 0)
        };

        lblPing = new XNALabel(WindowManager)
        {
            ClientRectangle = new Rectangle(6, 126, 0, 0)
        };

        lblPlayers = new XNALabel(WindowManager)
        {
            ClientRectangle = new Rectangle(6, 150, 0, 0)
        };

        lblPlayerNames = new XNALabel[MAX_PLAYERS];
        for (int i = 0; i < lblPlayerNames.Length / 2; i++)
        {
            XNALabel lblPlayerName1 = new(WindowManager)
            {
                ClientRectangle = new Rectangle(lblPlayers.X, lblPlayers.Y + 24 + (i * 20), 0, 0),
                RemapColor = UISettings.ActiveSettings.AltColor
            };

            XNALabel lblPlayerName2 = new(WindowManager)
            {
                ClientRectangle = new Rectangle(lblPlayers.X + 115, lblPlayerName1.Y, 0, 0),
                RemapColor = UISettings.ActiveSettings.AltColor
            };
