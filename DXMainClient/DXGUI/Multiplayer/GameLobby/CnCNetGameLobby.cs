...
public class CnCNetGameLobby : MultiplayerGameLobby
{
    ...
    /// <summary>
    /// The game mode of the latest selected map. Used for map sharing.
    /// </summary>
    private string lastGameMode;

    /// <summary>
    /// The map name of the latest selected map. Used for map sharing.
    /// </summary>
    private string lastMapName;

    /// <summary>
    /// The SHA1 of the latest selected map. Used for map sharing.
    /// </summary>
        ...
        : base(windowManager, iniName, topBar, mapLoader, discordHandler)
    {
        ...
        AddChatBoxCommand(new ChatBoxCommand(
            "TUNNELINFO",
            "View tunnel server information".L10N("UI:Main:CommandTunnelInfo"),
            false,
        ...
        AddChatBoxCommand(new ChatBoxCommand(
            ...
            true,
            (s) => ShowTunnelSelectionWindow("Select tunnel server:".L10N("UI:Main:SelectTunnelServer"))));
    }
    ...
    protected override void BroadcastPlayerOptions()
    {
        ...
        foreach (PlayerInfo pInfo in Players.Concat(AIPlayers))
        {
            ...
            // Combine the options into one integer to save bandwidth in cases where the player uses
            // default options (this is common for AI players) Will hopefully make GameSurge kicking
            // people a bit less common
    ...
    /// <summary>
    /// Signals other players that the local player has returned from the game, and unlocks the game
    /// as well as generates a new random seed as the game host.
    /// </summary>
    ...
    protected override void OnGameOptionChanged()
    {
        ...
        // We don't gain much in most cases by packing the drop-down values (because they're bytes
        // to begin with, and usually non-zero), so let's just transfer them as usual
    ...
    protected override void RequestPlayerOptions(int side, int color, int start, int team)
    {
        ...
        channel.SendCTCPMessage(
            ...
            QueuedMessageType.GAMESETTINGSMESSAGE,
            6);
    }
    ...
    private void ApplyGameOptions(string sender, string message)
    {
        ...
        // By changing the game options after changing the map, we know which game options were
        // changed by the map and which were changed by the game host

        // If the map doesn't exist on the local installation, it's impossible to know which options
        // were set by the host and which were set by the map, so we'll just assume that the host
        // has set all the options. Very few (if any) custom maps force options, so it'll be correct
        // nearly always
        for (int i = 0; i < checkBoxIntegerCount; i++)
        {
            ...
            if (!success)
            {
                AddNotice(
                    ...
                    "The game host's game version might be different from yours.").L10N("UI:Main:HostCheckBoxParseError"),
                    Color.Red);
                return;
            }
        ...
        for (int i = checkBoxIntegerCount; i < DropDowns.Count + checkBoxIntegerCount; i++)
        {
            ...
            if (!success)
            {
                AddNotice(
                    ...
                    "The game host's game version might be different from yours.").L10N("UI:Main:HostGameOptionInvalidTheSecondTime"),
                    Color.Red);
                return;
            }
        ...
        if (!parseSuccess)
        {
            AddNotice(
                ...
                "The game host's game version might be different from yours.").L10N("UI:Main:HostRandomSeedError"),
                Color.Red);
        }
    ...
    private void ApplyPlayerOptions(string sender, string message)
    {
        ...
        for (int i = 0; i < parts.Length;)
        {
            ...
            else
            {
                ...
                // If we can't find the player from the channel user list, ignore the player They've
                // either left the channel or got kicked before the player options message reached us
            ...
            else
            {
                ...
                if (pInfo.Name == ProgramConstants.PLAYERNAME)
                    BtnLaunchGame.Text = pInfo.Ready ? TextLaunchNotReady : TextLaunchReady;
    ...
    private void HandleMapSharingBlockedMessage(string sender)
    {
        AddNotice(string.Format(
            ...
            "they will be unable to participate in this match.".L10N("UI:Main:PlayerMissingMaDisabledSharing"),
            sender));
    }
        ...
        if (lastMapSHA1 == sha1)
        {
            if (!IsHost)
            {
                AddNotice(string.Format(
                    ...
                    "The host needs to change the map or {0} won't be able to participate in this match.".L10N("UI:Main:HostNeedChangeMapForPlayer"),
                    sender));
            }
            else
            {
                AddNotice(string.Format(
                    "{0} has failed to download the map from the CnCNet map database.".L10N("UI:Main:PlayerDownloadMapFailed") + " " +
                    "You need to change the map or {0} won't be able to participate in this match.".L10N("UI:Main:YouNeedChangeMapForPlayer"),
                    sender));
            }
        }
    }
