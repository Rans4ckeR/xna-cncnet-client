...
internal class LANLobby : XNAWindow
{
    ...
    public override void Initialize()
    {
        ...
        {
            Name = "lblColor",
            ClientRectangle = new Rectangle(lbChatMessages.X, 14, 0, 0),
            FontIndex = 1,
            Text = "YOUR COLOR:".L10N("UI:Main:YourColor")
        };
        ...
        DarkeningPanel gameCreationPanel = new(WindowManager);
    ...
    public void Open()
    {
        ...
        try
        {
            ...
            socket.Bind(new IPEndPoint(IPAddress.Any, ProgramConstants.LANLOBBYPORT));
            endPoint = new IPEndPoint(IPAddress.Broadcast, ProgramConstants.LANLOBBYPORT);
            initSuccess = true;
        }
    ...
    private void HandleNetworkMessage(string data, IPEndPoint endPoint)
    {
        ...
        string[] parameters = data.Substring(command.Length + 1).Split(
            new char[] { ProgramConstants.LANDATASEPARATOR },
    ...
    private void LbGameList_DoubleLeftClick(object sender, EventArgs e)
    {
        ...
        try
        {
            TcpClient client = new(hg.EndPoint.Address.ToString(), ProgramConstants.LANGAMELOBBYPORT);

            byte[] buffer;

            if (hg.IsLoadedGame)
            {
                IniFile spawnSGIni = new(ProgramConstants.GamePath +
                ...
                buffer = encoding.GetBytes("JOIN" + ProgramConstants.LANDATASEPARATOR +
                    ProgramConstants.PLAYERNAME + ProgramConstants.LANDATASEPARATOR +
                    loadedGameId + ProgramConstants.LANMESSAGESEPARATOR);
            ...
            else
            {
                ...
                buffer = encoding.GetBytes("JOIN" + ProgramConstants.LANDATASEPARATOR +
                    ProgramConstants.PLAYERNAME + ProgramConstants.LANMESSAGESEPARATOR);
