...
using InvitationIndex = System.Collections.Generic.Dictionary<System.Tuple<string, string>, System.WeakReference>;
using UserChannelPair = System.Tuple<string, string>;
...
internal class CnCNetLobby : XNAWindow, ISwitchable
{
    ...
    private readonly List<string> followedGames = new();
        ...
        : base(windowManager)
    {
        ...
        {
            new StringCommandHandler(ProgramConstants.GAMEINVITECTCPCOMMAND, HandleGameInviteCommand),
            new NoParamCommandHandler(ProgramConstants.GAMEINVITATIONFAILEDCTCPCOMMAND, HandleGameInvitationFailedNotification)
        };
    ...
    public override void Initialize()
    {
        ...
        btnNewGame.ClientRectangle = new Rectangle(12, Height - 29, UIDesignConstants.ButtonWidth133, UIDesignConstants.ButtonHeight);
        ...
        btnJoinGame.ClientRectangle = new Rectangle(
            btnNewGame.Right + 12,
            btnNewGame.Y,
        ...
        btnLogout.ClientRectangle = new Rectangle(
            Width - 145,
            btnNewGame.Y,
            UIDesignConstants.ButtonWidth133,
            UIDesignConstants.ButtonHeight);
        ...
        Rectangle gameListRectangle = new(
            btnNewGame.X,
            41,
        ...
        {
            ClientRectangle = gameListRectangle
        };
        ...
        lblColor.ClientRectangle = new Rectangle(
            lbChatMessages.X,
            14,
            0,
            0);
        ...
        foreach (IRCColor color in connectionManager.GetIRCColors())
        {
            ...
            XNADropDownItem ddItem = new()
            {
                Text = color.Name,
                TextColor = color.XnaColor,
                Tag = color
            };
        ...
        btnGameSortAlpha.ClientRectangle = new Rectangle(
            tbGameSearch.X + tbGameSearch.Width + 10,
            tbGameSearch.Y,
            21,
            21);
        ...
        btnGameFilterOptions.ClientRectangle = new Rectangle(
            btnGameSortAlpha.X + btnGameSortAlpha.Width + 10,
            tbGameSearch.Y,
            21,
            21);
    ...
    private void JoinGame_Internal(HostedCnCNetGame hg, string password)
    {
    ...
    private void BtnNewGame_LeftClick(object sender, EventArgs e)
    {
        ...
        GameCreationWindow gcw = (GameCreationWindow)gameCreationPanel.Tag;
    ...
    /// <summary>
    /// Displays a message when the IRC server has informed that the local user has been banned from
    /// a channel that they're attempting to join.
    /// </summary>
    private void ConnectionManager_BannedFromChannel(object sender, ChannelEventArgs e)
    {
        GenericHostedGame game = lbGameList.HostedGames.Find(hg => ((HostedCnCNetGame)hg).ChannelName == e.ChannelName);
    ...
    {
        IRCUser ircUser = e.User.IRCUser;
        XNAListBoxItem item = lbPlayerList.Items.Find(i => i.Text.StartsWith(ircUser.Name));

        item.Texture = ircUser.GameID < 0 || ircUser.GameID >= gameCollection.GameList.Count
            ? unknownGameIcon
            : gameCollection.GameList[ircUser.GameID].Texture;
    }
    ...
    private void DdCurrentChannel_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (currentChatChannel != null)
        {
            ...
            {
                // Remove the assigned channels from the users so we don't have ghost users on the
                // PM user list
        ...
        currentChatChannel.Messages.ForEach(AddMessageToChat);
    ...
    {
        List<UserChannelPair> toDismiss = new();
    ...
    private void DismissInvitation(UserChannelPair invitationIdentity)
    {
        if (invitationIndex.ContainsKey(invitationIdentity))
        {
            ...
            _ = invitationIndex.Remove(invitationIdentity);
        }
    }

    private HostedCnCNetGame FindGameByChannelName(string channelName)
    {
        GenericHostedGame game = lbGameList.HostedGames.Find(hg => ((HostedCnCNetGame)hg).ChannelName == channelName);
        if (game == null)
            return null;
    ...
    private void GameBroadcastChannel_CTCPReceived(object sender, ChannelCTCPEventArgs e)
    {
            ...
            e.Message.Length > 7)
        {
            ...
            if (version != ProgramConstants.GameVersion)
            {
                XNAMessageBox updateMessageBox = XNAMessageBox.ShowYesNoDialog(
                    WindowManager,
                    "Update available".L10N("UI:Main:UpdateAvailableTitle"),
        ...
        try
        {
            ...
            if (revision != ProgramConstants.CNCNETPROTOCOLREVISION)
                return;
            ...
            game.Incompatible = cncnetGame == localGame && game.GameVersion != ProgramConstants.GameVersion;
            ...
            // Seek for the game in the internal game list based on the name of its host; if found,
            // then refresh that game's information, otherwise add as new game
    ...
    /// <summary>
    /// Removes a game from the list when the host quits CnCNet or leaves the game broadcast channel.
    /// </summary>
    ...
    private void Gcw_LoadedGameCreated(object sender, GameCreationEventArgs e)
    {
        ...
        gameLoadingChannel.UserAdded += GameLoadingChannel_UserAdded;
        connectionManager.SendCustomMessage(new QueuedMessage(
            "JOIN " + channelName + " " + e.Password,
            QueuedMessageType.INSTANTMESSAGE,
            0));
        connectionManager.MainChannel.AddMessage(message: new ChatMessage(
            Color.White,
            string.Format("Creating a game named {0} ...".L10N("UI:Main:CreateGameNamed"), e.GameRoomName)));
    ...
    /// </summary>
    /// <param name="hg"><see cref="HostedCnCNetGame"/>.</param>
    /// <returns>result.</returns>
    ...
    /// <summary>
    /// Checks if the user can join a game. Returns null if the user can, otherwise returns an error
    /// message that tells the reason why the user cannot join the game.
    /// </summary>
    ...
    private void HandleGameInvitationFailedNotification(string sender)
    {
        ...
        if (isInGameRoom && !ProgramConstants.IsInGame)
        {
            gameLobby.AddWarning(
                string.Format(
                    "{0} could not receive your invitation. They might be in game " +
                        "or only accepting invitations from friends. Ensure your game is " +
                        "unlocked and visible in the lobby before trying again.".L10N("UI:Main:InviteNotDelivered"),
                    sender));
        }
    }
    ...
    private void HandleGameInviteCommand(string sender, string argumentsString)
    {
        ...
        int gameIndex = lbGameList.HostedGames.FindIndex(hg => ((HostedCnCNetGame)hg).ChannelName == channelName);

        // also enforce user preference on whether to accept invitations from non-friends this is
        // kept separate from CanReceiveInvitationMessagesFrom() as we still want to let the host
        // know that we couldn't receive the invitation
        ...
        {
            // let the host know that we can't accept note this is not reached for the rejection case
            connectionManager.SendCustomMessage(new QueuedMessage(
                "PRIVMSG " + sender + " :\u0001" +
                ProgramConstants.GAMEINVITATIONFAILEDCTCPCOMMAND + "\u0001",
        ...
        // if there's already an outstanding invitation from this user/channel combination, we don't
        // want to display another we won't bother telling the host though, since their old
        // invitation is still available to us
        UserChannelPair invitationIdentity = new(sender, channelName);
        ...
        ChoiceNotificationBox gameInviteChoiceBox = new(WindowManager);

        WindowManager.AddAndInitializeControl(gameInviteChoiceBox);

        // show the invitation at top left; it will remain until it is acted upon or the target game
        // is closed
        gameInviteChoiceBox.Show(
            ...
            "Yes".L10N("UI:Main:ButtonYes"),
            "No".L10N("UI:Main:ButtonNo"),
            0);

        // add the invitation to the index so we can remove it if the target game is closed also
        // lets us silently ignore new invitations from the same person while this one is still outstanding
        invitationIndex[invitationIdentity] =
            new WeakReference(gameInviteChoiceBox);

        gameInviteChoiceBox.AffirmativeClickedAction = choiceBox =>
        {
        ...
        gameInviteChoiceBox.NegativeClickedAction = choiceBox =>
        {
            // clean up the index as this invitation no longer exists
            _ = invitationIndex.Remove(invitationIdentity);
        };
    ...
    private bool HostedGameMatches(GenericHostedGame hg)
    {
        ...
        return
            ...
            hg.GameMode.ToUpper().Equals(tbGameSearch.Text.ToUpper(), StringComparison.Ordinal) ||
            hg.Map.ToUpper().Contains(tbGameSearch.Text.ToUpper()) ||
            hg.Players.Any(pl => pl.ToUpper().Equals(tbGameSearch.Text.ToUpper(), StringComparison.Ordinal));
    }
    ...
    private void InitializeGameList()
    {
        ...
        foreach (CnCNetGame game in gameCollection.GameList)
        {
            ...
            if (chatChannel == null)
            {
                chatChannel = connectionManager.CreateChannel(
                    game.UIName,
                    game.ChatChannel,
                    true,
                    true,
    ...
    private void Instance_SettingsSaved(object sender, EventArgs e)
    {
        ...
        foreach (CnCNetGame game in gameCollection.GameList)
        {
                ...
                !UserINISettings.Instance.IsGameFollowed(game.InternalName.ToUpper()))
            {
                ...
                _ = followedGames.Remove(game.InternalName);
            }
    ...
    private bool JoinGame(HostedCnCNetGame hg, string password, IMessageView messageView)
    {
        ...
        // if (hg.GameVersion != ProgramConstants.GAME_VERSION) TODO Show warning
        if (hg.Passworded)
        {
    ...
    {
        HostedCnCNetGame listedGame = (HostedCnCNetGame)lbGameList.SelectedItem?.Tag;
        if (listedGame == null)
            return;
        int hostedGameIndex = lbGameList.HostedGames.IndexOf(listedGame);
        _ = JoinGameByIndex(hostedGameIndex, string.Empty);
    }
    ...
    private void JoinUser(IRCUser user, IMessageView messageView)
    {
        ...
        HostedCnCNetGame game = GetHostedGameForUser(user);
        if (game == null)
        {
        ...
        _ = JoinGame(game, string.Empty, messageView);
    }

    private void LbChatMessages_RightClick(object sender, EventArgs e)
    {
        XNAListBoxItem item = lbChatMessages.HoveredItem;
        ChatMessage chatMessage = item?.Tag as ChatMessage;
    ...
    private void LbPlayerList_DoubleLeftClick(object sender, EventArgs e)
    {
        ...
        ChannelUser channelUser = (ChannelUser)lbPlayerList.SelectedItem.Tag;
    ...
    /// <summary>
    /// Hides the login window and the CnCNet lobby if the user cancels connecting to CnCNet in the
    /// login dialog.
    /// </summary>
    ...
    private void PasswordRequestWindow_PasswordEntered(object sender, PasswordEventArgs e) => JoinGame_Internal(e.HostedGame, e.Password);
    ...
    private void PostUIInit()
    {
        ...
        GameCreationWindow gcw = new(WindowManager, tunnelHandler);
        ...
        DarkeningPanel loginWindowPanel = new(WindowManager)
        {
            Alpha = 0.0f
        };
        ...
        DarkeningPanel passwordRequestWindowPanel = new(WindowManager)
        {
            Alpha = 0.0f
        };
