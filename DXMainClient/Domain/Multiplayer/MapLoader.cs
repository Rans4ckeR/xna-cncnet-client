...
public class MapLoader
{
    ...
    {
        if (!File.Exists(ProgramConstants.GamePath + mapPath + MAPFILEEXTENSION))
        {
            Logger.Log("LoadCustomMap: Map " + mapPath + " not found!");
            resultMessage = $"Map file {mapPath}{MAPFILEEXTENSION} doesn't exist!";

            return null;
        }

        Logger.Log("LoadCustomMap: Loading custom map " + mapPath);
        string iniPath = ProgramConstants.GamePath + mapPath + MAPFILEEXTENSION;
        Map map = new(mapPath, iniPath);
        ...
        if (map.SetInfoFromCustomMap())
        {
            ...
            IEnumerable<GameMode> gameModes = GameModes.Where(gm => gm.Maps.Contains(map));
    ...
    private void LoadCustomMaps()
    {
        ...
        ConcurrentDictionary<string, Map> customMapCache = MapLoader.LoadCustomMapCache();
        List<string> localMapSHAs = new();
        ...
        foreach (string missingSHA in customMapCache.Keys.Where(cachedSHA => !localMapSHAs.Contains(cachedSHA)))
        {
            _ = customMapCache.TryRemove(missingSHA, out _);
        }

        // save cache
        MapLoader.CacheCustomMaps(customMapCache);
