...
public static class MapSharer
{
    ...
    private static string ExtractZipFile(string zipFile, string destDir)
    {
        ...
        // here, we extract every entry, but we could extract conditionally based on entry name,
        // size, date, checkbox status, etc.
        zipArchive.ExtractToDirectory(destDir);

        return zipArchive.Entries.FirstOrDefault()?.Name;
    }

    private static string MapUpload(string url, Map map, string gameName, out bool success)
    {
        ...
        // Upload the file to the URI. The 'UploadFile(uriString,fileName)' method implicitly uses
        // HTTP POST method.
        try
        {
            ...
            byte[] responseArray = UploadFiles(url, files, values);
