...
public static class MapPreviewExtractor
{
    ...
    public static Bitmap ExtractMapPreview(IniFile mapIni)
    {
        ...
        string baseFilename = mapIni.FileName.Replace(ProgramConstants.GamePath, string.Empty);
        ...
        string[] previewSizes = mapIni.GetStringValue("Preview", "Size", string.Empty).Split(',');
        ...
        if (sectionKeys != null)
        {
            foreach (string key in sectionKeys)
                _ = sb.Append(mapIni.GetStringValue("PreviewPack", key, string.Empty));
        }
    ...
    private static byte[] DecompressPreviewData(byte[] dataSource, int decompressedDataSize, out string errorMessage)
    {
        try
        {
            ...
            while (true)
            {
                ...
                LzoStream stream = new(new MemoryStream(dataSource, readBytes, sizeCompressed), CompressionMode.Decompress);
                _ = stream.Read(dataDest, writtenBytes, sizeUncompressed);
