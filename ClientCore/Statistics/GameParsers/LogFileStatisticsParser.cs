...
public class LogFileStatisticsParser : GenericMatchParser
{
    ...
    protected override void ParseStatistics(string gamepath)
    {
        ...
        try
        {
            StreamReader reader = new(File.OpenRead(gamepath + fileName));
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                else if (line.Contains("Game loop finished. Average FPS"))
                {
                    ...
                    Statistics.AverageFPS = int.Parse(fpsString);
                }
                ...
                if (line.StartsWith("Lost = "))
                    currentPlayer.Losses = int.Parse(line.Substring(7));
                else if (line.StartsWith("Kills = "))
                    currentPlayer.Kills = int.Parse(line.Substring(8));
                else if (line.StartsWith("Score = "))
                    currentPlayer.Score = int.Parse(line.Substring(8));
                else if (line.StartsWith(economyString + " = "))
                    currentPlayer.Economy = int.Parse(line.Substring(economyString.Length + 2));
            }
