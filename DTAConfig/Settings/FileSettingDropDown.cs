...
{
    private readonly List<List<FileSourceDestinationInfo>> itemFilesList = new();

    public FileSettingDropDown(WindowManager windowManager)
        : base(windowManager)
    {
    }
    ...
    public override void GetAttributes(IniFile iniFile)
    {
        ...
        IniSection section = iniFile.GetSection(Name);
        if (section == null)
            return;
    ...
    public bool RefreshSetting()
    {
        ...
        if (CheckAvailability)
        {
            for (int i = 0; i < Items.Count; i++)
            {
                ...
                foreach (FileSourceDestinationInfo fileInfo in itemFilesList[i])
                {
